...
const Login = async (req, res) =>{
    ...
    if(match && user.status == true){
        ...
        res.cookie('jwt', token, {httpOnly: true, secure: false, sameSite: 'None',maxAge: 24 * 60 * 60 * 1000 })
        res.json({token})                
...
const forgetPassword = async (req, res)=>{
    ...
    if(!user){
     ...
     let mailer = {
         ...
         <a href="http://localhost:3000/api/auth/resetpassword/${token}">Cliquer pour verifier </a>`
     }
     await transporter.sendMail(mailer)
    }  
}
